...
class sale_forecast(models.Model):
    ...
    period = fields.Selection([('week', 'Week'), ('month', 'Month'), ('quarter', 'Quarter'), ('year', 'Year')],
                              'Period', required=True, copy=False, default='week')
    ...
    def get_period(self, period, start_date, period_count):
        ...
        if period == 'week':
            for item in range(period_count):
                ...
                new_start_date = datetime.strptime(new_week_date, DEFAULT_SERVER_DATE_FORMAT) + relativedelta(
            ...
            for item in range(period_count):
                ...
                new_start_date = datetime.strptime(new_month_date, DEFAULT_SERVER_DATE_FORMAT) + relativedelta(
            ...
            for item in range(period_count):
                ...
                new_start_date = datetime.strptime(new_quarter_date, DEFAULT_SERVER_DATE_FORMAT) + relativedelta(
            ...
            for item in range(period_count):
                ...
                new_start_date = datetime.strptime(new_year_date, DEFAULT_SERVER_DATE_FORMAT) + relativedelta(
    ...
    def generate_forecast(self):
        ...
        for index, item in enumerate(periods):
            if self.period == 'week':
                ...
                previous_rec_date = datetime.strptime(self.start_date, DEFAULT_SERVER_DATE_FORMAT) - relativedelta(
                    weeks=1) - relativedelta(days=1)
                previous_rec_start_date = previous_rec_date.date().strftime(DEFAULT_SERVER_DATE_FORMAT)
                sec_last_period_date = datetime.strptime(self.start_date, DEFAULT_SERVER_DATE_FORMAT) - relativedelta(
                    weeks=2) - relativedelta(days=2)
                sec_last_start_date = sec_last_period_date.date().strftime(DEFAULT_SERVER_DATE_FORMAT)
                third_last_period_date = datetime.strptime(self.start_date, DEFAULT_SERVER_DATE_FORMAT) - relativedelta(
            ...
            for product in self.product_ids:
                ...
                if routes:
                    if routes.filtered(lambda r: r.id == mto_route_id):
                        ...
                        if routes.filtered(lambda r: r.name == 'Buy') and routes.filtered(
                ...
                ctx.update({'from_date': item.name, 'to_date': end_date, 'warehouse': self.warehouse_id.id,
                            'current_model': 'sale.forecast', 'rec_id': self.id})
                product_qty = self.env['product.product'].browse(
                    product.id)  # _product_available(self._cr,1, [product.id], False, False, ctx)
                ...
                if first_period == True:
                    ...
                    forecast_product_last_recs = forecast_product_rec.search(
                        [('period_start_date', '=', previous_rec_start_date), ('product_id', '=', product.id)])
                    ...
                    forecast_product_sec_last_recs = forecast_product_rec.search(
                        [('period_start_date', '=', sec_last_start_date), ('product_id', '=', product.id)])
                    ...
                    forecast_product_third_last_recs = forecast_product_rec.search(
                        [('period_start_date', '=', third_last_start_date), ('product_id', '=', product.id)])
                    ...
                    for i in range(1, 12):
                        if self.period == 'week':
                            previous_new_rec_date = datetime.strptime(previous_start_date,
                        ...
                        order_recs = order_obj.search(
                            [('date_order', '<=', previous_start_date), ('date_order', '>=', previous_new_rec_date)])
                        if order_recs:
                            for order_rec in order_recs:
                                order_line_recs = order_line_obj.search(
                                    [('order_id', '=', order_rec[0].id), ('product_id', '=', product.id)])
                                if order_line_recs:
    ...
    def update_action_qty(self):
        ...
        if self.forecast_filter_id:
            recs = self.env['forecast.product'].search(
                [('forecast_id', '=', self.id), ('period_start_date', '=', self.forecast_filter_id.name)])
        ...
        for rec in recs:
            ...
            if not rec.onhand_qty:
                ...
                new_rest_period_qty = self.env['forecast.product'].search(
                    [('product_id', '=', rec.product_id.id), ('period_end_date', '=', previous_period_end_date),
                     ('forecast_id', '=', self.id)]).action_qty
            ...
            for line in self.forecast_product_ids:
                ...
                if line_route_ids:
                    if not line_route_ids.filtered(
                            lambda r: r.id == mto_route_id) and line.action_qty > 0 and line.action_required != 'none':
                        ...
                        if line_prod_temp_id.type == 'product':
                            if line.action_required == 'buy':
                                ...
                                routes = route_id and [(4, route_id)] or []
                                picking_type_id = self.env['stock.warehouse'].search(
                                    [('id', '=', self.warehouse_id.id)]).int_type_id
                                location_id = self.env['stock.picking.type'].search(
                                    [('id', '=', picking_type_id.id)]).default_location_dest_id

                            # routes = line_route_ids and [(4, line_route_ids.id)] or []
                            date_planned = datetime.strptime(line.period_start_date,
                                                             DEFAULT_SERVER_DATE_FORMAT) - relativedelta(
                                days=company_rec.security_lead)
                            procurement_date_planned = date_planned.strftime(DEFAULT_SERVER_DATE_FORMAT)
                            if not location_id:
                                UserError(
                                    'Does not have a defined location, action required must be (buy,manufacture or both).')
                            vals = {
                                # 'product_uos_qty': 
                            ...
                            if line.action_required == 'buy':
                                if procurement_order_rec_state == 'exception':
                                    ...
                                    purchase_order_rec = procurement_order_obj.search(
                                        [('id', '=', procurement_id)]).purchase_id
                                    document_number = self.env['purchase.order'].search(
                                        [('id', '=', purchase_order_rec.id)]).name
                                ...
                                if procurement_order_rec_state == 'exception':
                                    ...
                                    mrp_production_rec = procurement_order_obj.search(
                                        [('id', '=', procurement_id)]).production_id
                                    document_number = self.env['mrp.production'].search(
                                        [('id', '=', mrp_production_rec.id)]).name
...
class product_product(models.Model):
    ...
    def _get_domain_dates(self):
        for record in self:
            if (self._context.has_key('current_model') and self._context['current_model']) and (
                    self._context.has_key('rec_id') and self._context['rec_id']):
...
class forecast_product(models.Model):
    ...
    action_required = fields.Selection(
        [('buy', 'Buy'), ('manufacture', 'Manufacture'), ('both', 'Both'), ('none', 'Not Required')], 'Action Required',
        copy=False)
